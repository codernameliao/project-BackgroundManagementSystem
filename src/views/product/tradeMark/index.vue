<template>
  <div>
    ...
    <!-- 表格组件 
         ...
         align：标题的对齐方式
         prop:对应列内容的字段名
         注意1：elmentUI当中的table组件，展示的数据是以一列一列el-table-column进行展示数据，表格有4列，第一列 label="序号"展示标题，
         type="index"展示序号。第二列label="品牌名称"展示品牌名称，prop="tmName"展示tmName的属性值，
         :data展示的是数组，以每一行显示，list数组有3条数据，表格就展示3行
       -->
    <el-table style="width: 100%" border :data="list">
      ...
      <!-- prop="logoUrl"展示图片地址，需要用到作用域插槽（子组件的数据来源于父组件，而子组件是决定不了它的结构与外观） -->
      <!-- 作用域插槽将数据进行回传给父组件，slot-scope="{ row, $index }"，row返回list数组的每一条数组， $index返回下标 -->
...
export default {
  ...
  methods: {
    ...
    updateTradeMark(row) {
      ...
      //也就是tmForm存储即为服务器返回品牌信息，this.tmForm = row ;不能直接赋值，需要浅拷贝
      this.tmForm = { ...row };
    },
